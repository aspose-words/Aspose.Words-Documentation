...
description: "Aspose.Words for C++ {enables|allows} you to set advanced properties when loading a document. This {allows|enables} you to more accurately control the load process."
type: docs
weight: 10
url: /cpp/specify-load-options/
---

When loading a document, you can set some advanced properties. Aspose.Words provides you with the [LoadOptions](https://apireference.aspose.com/words/cpp/class/aspose.words.loading.load_options) class, which {allows|enables} more precise control of the load process. Some load formats have a corresponding class that holds load options for this load format, for example, there is [PdfLoadOptions](https://apireference.aspose.com/words/cpp/class/aspose.words.loading.pdf_load_options) for loading to PDF format or [TxtLoadOptions](https://apireference.aspose.com/words/cpp/class/aspose.words.loading.txt_load_options) for loading to TXT. This article provides examples of working with options of the **LoadOptions** class.
...
A document may contain external links to images located somewhere on a local disk, network, or Internet. Aspose.Words automatically loads such images into a document, but there are situations when this process needs to be controlled. For example, to decide whether we really need to load a certain image or perhaps skip it. The ResourceLoadingCallback load option {allows|enables} you to control this.

The following code example {shows|demonstrates} the implementation of the IResourceLoadingCallback interface:
