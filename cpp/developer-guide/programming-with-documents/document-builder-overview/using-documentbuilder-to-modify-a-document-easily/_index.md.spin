...
The shading texture is set with a TextureIndex enumeration value that enables the application of various patterns to the **Shading** object. For example, to set a background color for a document element, use the TextureIndex.TextureSolid value and set the foreground shading color as appropriate. Below example shows how to apply borders and shading to a paragraph.
...
Below example demonstrates how to move a cursor position to just after the bookmark end.
...
**ConvertUtil** is very useful when setting different page properties because for instance inches are more usual measurement units than points. The following example shows how to set up the page properties in inches. Below example demonstrates how to specify page properties in inches.
