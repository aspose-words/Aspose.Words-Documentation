...
Current font formatting is represented by a Font object returned by the DocumentBuilder.Font property. The Font class contains a wide variety of the font properties possible in Microsoft Word. Below example {shows|demonstrates} how to set font formatting.
...
The following code example {demonstrates|shows} how to change Asian paragraph spacing and indents.

{{< gist "aspose-com-gists" "518f03cac02abb105e02f55edb7de9f9" "cpp-Programming-Documents-Document-DocumentBuilderSetFormatting-DocumentBuilderSetSpacebetweenAsianandLatintext.cpp" >}}

#### Set Line Break Options

Asian Typography tab of the Paragraph properties dialog in MS Word has line break group. The options of this group can be set using FarEastLineBreakControl, WordWrap, HangingPunctuation properties of ParagraphFormat class. Below code example {shows|demonstrates} how to use these properties.
...
Current row formatting is determined by a RowFormat object that is returned by the DocumentBuilder.RowFormat property. The object encapsulates information about all table row formatting. Below example {shows|demonstrates} how to create a table that contains a single cell and apply row formatting.
...
Aspose.Words {allows|enables} the easy creation of lists by applying list formatting. DocumentBuilder provides the DocumentBuilder.ListFormat property that returns a ListFormat object. This object has {several|multiple|a few|many|numerous} methods to start and end a list and to increase/decrease the indent. There are two general types of lists in Microsoft Word: bulleted and numbered.
...
Below example {shows|demonstrates} how to build a multilevel list.
...
The shading texture is set with a TextureIndex enumeration value that {allows|enables} the application of various patterns to the **Shading** object. For example, to set a background color for a document element, use the TextureIndex.TextureSolid value and set the foreground shading color as appropriate. Below example {shows|demonstrates} how to apply borders and shading to a paragraph.
...
Also, you can use the DocumentBuilder.CurrentParagraph property, which gets the paragraph that is currently selected in this **DocumentBuilder**. Below example {shows|demonstrates} how to access the current node in a document builder. You can download template file of this example from here.
...
If you are working with a document that contains {multiple|several|a few|many|numerous} sections, you can move to a desired section using DocumentBuilder.MoveToSection. This method moves the cursor to the beginning of a specified section and accepts the index of the required section. When the section index is greater than or equal to 0, it specifies an index from the beginning of the document with 0 being the first section. When the section index is less than 0, it specifies an index from the end of the document with -1 being the last section. Below example {shows|demonstrates} how to move a cursor position to the specified section. You can download template file of this example from here.
...
The character index can currently only be specified as 0 to move to the beginning of the paragraph or -1 to move to the end of the paragraph. Below example {shows|demonstrates} how to move a cursor position to the specified paragraph. You can download template file of this example from here.
...
Also, note that characterIndex currently can only specify 0 to move to the beginning of the cell or -1 to move to the end of the cell. Below example {shows|demonstrates} how to move a cursor position to the specified table cell. You can download template file of this example from here.
...
Below example {shows|demonstrates} how to move a cursor position to just after the bookmark end.
