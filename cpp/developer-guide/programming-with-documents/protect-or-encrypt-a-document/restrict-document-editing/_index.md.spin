...
description: "Aspose.Words for C++ {allows|enables} to restrict editing a document by setting a restriction type. You can also remove protection and make unrestricted editable regions."
type: docs
weight: 30
...
Aspose.Words {allows|enables} you to restrict editing a document by setting a restriction type. In addition, Aspose.Words also {enables|allows} you to specify write protection settings for a document.
...
Aspose.Words {allows|enables} you to control the way you restrict the content using the [ProtectionType](https://apireference.aspose.com/words/cpp/namespace/aspose.words#protectiontype) enumeration parameter. This will enable you to select an exact type of protection such as the following:
...
Note that the password being set is just a property in a document that can be removed if the document properties are accessed. Accordingly, such a password is not a guarantee of the document security. The [Unprotect](https://apireference.aspose.com/words/cpp/class/aspose.words.document#unprotect) method {shows|demonstrates} just that.
...
Aspose.Words {allows|enables} you to protect your documents from changes using the [Protect](https://apireference.aspose.com/words/cpp/class/aspose.words.document#protect_protectiontype) method. This method is not a security feature and does not encrypt a document.
...
The following code example {shows|demonstrates} how to add password protection to your document:
...
The following code example {shows|demonstrates} how to restrict editing in a document so only editing in form fields is possible:
...
Aspose.Words {allows|enables} you to mark the parts that can be changed in your document using the [StartEditableRange](https://apireference.aspose.com/words/cpp/class/aspose.words.document_builder#starteditablerange) and [EndEditableRange](https://apireference.aspose.com/words/cpp/class/aspose.words.document_builder#endeditablerange) methods.

The following code example {shows|demonstrates} how to mark the whole document as read-only and specify editable regions in it:
...
The following code example {shows|demonstrates} how to add a restriction for the entire document, and then remove the restriction for one of the sections:
