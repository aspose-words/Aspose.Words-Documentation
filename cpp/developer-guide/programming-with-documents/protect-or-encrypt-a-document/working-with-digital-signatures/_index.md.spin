...
description: "Aspose.Words for C++ {allows|enables} you to digitally sign documents and detect, count, verify, and remove existing digital signatures."
type: docs
weight: 160
...
Aspose.Words {allows|enables} you to work with digital signatures on DOC, OOXML, and ODT documents and to sign the generated document in PDF or XPS format.

## Limitations of Digital Signatures

The table below describes {a few|several|numerous} limitations that you may face while working with digital signatures through Aspose.Words, as well as some alternative options.
...
Aspose.Words {allows|enables} you to detect digital signature in a document using the the [DetectFileFormat](https://apireference.aspose.com/words/cpp/class/aspose.words.file_format_util#detectfileformat_stream) method and the [HasDigitalSignature](https://apireference.aspose.com/words/cpp/class/aspose.words.file_format_info#get_hasdigitalsignature_const) property. It is worth noting that such a check will only detect the fact of the signature, but not its validity.

A document can be signed more than once, and this can be done by {various|different} users. To check the validity of digital signatures, you need to load them from the document using the [LoadSignatures](https://apireference.aspose.com/words/cpp/class/aspose.words.digital_signatures.digital_signature_util#loadsignatures_stream) method and use the [IsValid](https://apireference.aspose.com/words/cpp/class/aspose.words.digital_signatures.digital_signature_collection#get_isvalid) property. Also Aspose.Words {allows|enables} you to count a set of all digital signatures within a document using the [Count](https://apireference.aspose.com/words/cpp/class/aspose.words.digital_signatures.digital_signature_collection#get_count) property.
...
Aspose.Words {allows|enables} you to sign and get all details of a PDF document using the [PdfDigitalSignatureDetails](https://apireference.aspose.com/words/cpp/class/aspose.words.saving.pdf_digital_signature_details) properties.
...
// The other constructor overloads can be used to load certificates from {various|different} locations.
auto certificateHolder = CertificateHolder::Create(u"morzal.pfx", u"aw");
...
// We can use this attribute to set a {various|different} hash algorithm.
...
Aspose.Words {allows|enables} you to remove all digital signatures from a signed document using the [RemoveAllSignatures](https://apireference.aspose.com/words/cpp/class/aspose.words.digital_signatures.digital_signature_util#removeallsignatures_stream_stream) method.
