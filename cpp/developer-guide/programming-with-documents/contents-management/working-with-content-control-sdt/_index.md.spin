...
The ControlChar class is a repository for the constants that represent control characters often encountered in documents. It provides both char and string versions of the same constants. For example, string ControlChar.LineBreak and char ControlChar.LineBreakChar has the same value. The below code example {shows|demonstrates} how to use control characters.
...
If you want to set the style of content control, you can use StructuredDocumentTag.Style or StructuredDocumentTag.StyleName properties.The below code example {shows|demonstrates} how to set the style of content control. When you type the text into content control in the output document, the typed text will have style "Quote".
