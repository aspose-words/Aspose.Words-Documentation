...
Flow layout does not support overlapping items (as well as Z-order at all) but in several cases it is possible to process elements correctly when 1) rectangles are exported as frames and 2) elements that overlap with a rectangle (frame) are “moved” into that frame (which becomes parent element).
