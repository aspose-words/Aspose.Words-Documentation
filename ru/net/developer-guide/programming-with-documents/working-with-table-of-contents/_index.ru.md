...
Первый призыв к **UpdateFields** будет строить <p notrans="<p notrans=" `TOC`"=""></p>"> Все тексты заполнены и `TOC` Кажется почти полным. Единственное, чего не хватает, так это номеров страниц, которые пока отображаются с "?". Второй призыв к **UpdatePageLayout** Построит макет документа в память. Это необходимо сделать, чтобы собрать номера страниц записей. Правильные номера страниц, рассчитанные по этому вызову, затем вставляются в TOC.
