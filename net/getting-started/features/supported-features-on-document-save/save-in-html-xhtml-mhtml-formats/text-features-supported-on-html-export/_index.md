...
Each Run node in the model is exported as a separate span to retain formatting properly. Some documents can contain a few runs that are unrequired and can be joined. In the resulting HTML document, this can result in a few extra span elements.
