...
Each Run node in the model is exported as a separate span to retain formatting properly. Some documents can contain multiple runs that are unrequired and can be joined. In the resulting HTML document, this can result in several extra span elements.
