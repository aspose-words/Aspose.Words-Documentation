...
The following code example {shows|demonstrates} how to convert DOCX to Markdown:
...
The following code example {shows|demonstrates} how to specify a folder for images when saving a document to a stream:
...
The following example {shows|demonstrates} how to create a document with some styles and save it to Markdown:
...
There are {several|multiple|a few|many|numerous} nuances and interesting cases, having learned which you can work with Markdown files more flexibly and conveniently. For example, there is the ability to use:

* SetextHeading that {allows|enables} you to create multi-line headings in Markdown, while regular headings in Markdown can only be single-line. SetextHeading is based on a "Heading N" style, and its level can only be 1 or 2. If N in "Heading N" is greater than or equal to 2, then the corresponding SetextHeading is based on "Heading 2", otherwise on "Heading 1".
