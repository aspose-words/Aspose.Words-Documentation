...
description: "Aspose.Words for .NET {allows|enables} you to split a document into {several|multiple|a few|many|numerous} files. Use the split feature to effectively split a document by headings or sections, as well as by pages or by page ranges."
type: docs
weight: 80
...
Aspose.Words provides you with an efficient way to split one document into {multiple|several|a few|many|numerous} documents by headings or sections. You can also split a document by pages or by page ranges. Both splitting options will be described in this article.
...
## Split a Document {Using|by} Different {Criteria|Options} {#split-a-document-using-different-criteria}

Aspose.Words {allows|enables} you to split EPUB or HTML documents into chapters according to various criteria. In the process, the style and layout of the source document are preserved for the output documents.
...
### Split a Document {by|Using} Headings {#split-a-document-by-headings}
...
The following code example {shows|demonstrates} how to split a document into smaller parts by heading:
...
When saving to EPUB, the document is not split into {several|multiple|a few|many|numerous} files, and there will be only one output file.

### Split a Document {by|Using} Sections {#split-a-document-by-sections}

Aspose.Words also {enables|allows} you to use section breaks to split documents and save them to HTML. For this purpose, use **SectionBreak** as the **DocumentSplitCriteria**:
...
There is another way to split the source document into {multiple|several|a few|many|numerous} output documents, and you can choose any output format supported by Aspose.Words.

The following code example {shows|demonstrates} how to split a document into smaller parts by section breaks (without using the DocumentSplitCriteria property):
...
## {Split|Splitting} by Pages {#splitting-by-pages}
...
This section describes {several|multiple|a few|many|numerous} use cases of dividing documents by paged using the [Document](https://apireference.aspose.com/words/net/aspose.words/document) class and the **ExtractPages** method.
...
Due to the {many|multiple|several|a few|numerous} nuances appearing while reducing the number of pages, a full match with the Microsoft Word layout is quite a complicated task. Therefore, depending on the document complexity, there may be slight differences in the resulting document layout from the original document.

{{% /alert %}}

### Split a Document {Page by Page|by Page} {#split-a-document-page-by-page}
...
### Split a Document {by|Using} Page Ranges {#split-a-document-by-page-ranges}

Aspose.Words {allows|enables} splitting a multi-page document by page ranges. You can split one file into {multiple|several|a few|many|numerous} files with various page ranges or just select one range and save only this part of the source document. Note that you can choose the page range according to the maximum and minimum page number of a document.

The following code example {shows|demonstrates} how to split a document into smaller parts by page range with specific start and end indexes:
...
## Callback Option {to Save|for Saving} a Document {#callback-option-for-saving-a-document}

You can use the [DocumentPartSavingCallback](https://apireference.aspose.com/words/net/aspose.words.saving/htmlsaveoptions/properties/documentpartsavingcallback) property to control how Aspose.Words saves document parts when this document is exported into HTML format. This property {allows|enables} you to rename output files or even to redirect them into custom streams.
...
## Merge the Split Document with Another {Document|File} {#merge-the-split-document-with-another-file}

Aspose.Words {enables|allows} you to merge the output split document with another document to form a new document. This can be called document merging.

The following code example {shows|demonstrates} how to merge a split document with another document:
