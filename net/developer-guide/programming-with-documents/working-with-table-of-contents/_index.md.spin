...
Often you will work with documents containing a table of contents (TOC). Using Aspose.Words you can insert your own table of contents or completely rebuild existing table of contents in the document using just {a few|multiple|several|many|numerous} lines of code. This article outlines how to work with the table of contents field and demonstrates:
...
Below code sample {shows|demonstrates} how to insert a Table of contents (TOC) into a document using heading styles as entries.
...
Aspose.Words {allows|enables} you to completely update a TOC with only {a few|multiple|several|many|numerous} lines of code. This can be done to populate a newly inserted TOC or to update an existing TOC after changes to the document have been made. The following two methods must be used in order to update the TOC fields in the document:

1. Document.UpdateFields
1. Document.UpdatePageLayout

Please note that these two update methods are required to be called in that order. If reversed the table of contents will be populated but no page numbers will be displayed. Any number of different TOCs can be updated. These methods will automatically update all TOCs found in the document. Below code sample {shows|demonstrates} how to completely rebuild TOC fields in the document by invoking field update.
...
![working-with-table-of-contents-aspose-words-net](working-with-table-of-contents_1.png)
...
|**Preserve New Line Entries** <br>*(\X Switch)* |<p>Similar to the switch above, this switch specifies that headings spanning over {multiple|several|a few|many|numerous} lines (using new line characters not separate paragraphs) will be preserved as they are in the generated TOC. For example, a heading which is to spread across {multiple|several|a few|many|numerous} lines can use the new line character (Ctrl + Enter or ControlChar.LineBreak) to separate content across different lines. With this switch specified, the entry in the TOC will preserve these new line characters as shown below. <br><br>In this situation if the switch is not defined then the new line characters are converted to a single white space. </p><p>![working-with-table-of-contents-aspose-words](working-with-table-of-contents_10.png)</p>|

### Insert TC Fields

You can insert a new TC field at the current position of the DocumentBuilder by calling the DocumentBuilder.InsertField method and specifying the field name as “TC” along with any switches that are needed. Below example {shows|demonstrates} how to insert a TC field into the document using DocumentBuilder.
...
Using the Style class retrieved for the particular TOC level you want to modify, you can also modify how these appear in the document. To change how this appears firstly Style.ParagraphFormat must be called to retrieve the paragraph formatting for the style. From this the tab stops can be retrieved by calling ParagraphFormat.TabStops and the appropriate tab stop modified. Using this same technique the tab itself can be moved or removed all together. Below example {shows|demonstrates} how to modify the position of the right tab stop in TOC related paragraphs.
...
A table of contents can be removed from the document by removing all nodes found between the FieldStart and FieldEnd node of the TOC field. The code below {demonstrates|shows} this. The removal of the TOC field is simpler than a normal field as we do not keep track of nested fields. Instead we check the FieldEnd node is of type FieldType.FieldTOC which means we have encountered the end of the current TOC. This technique can be used in this case without worrying about any nested fields as we can assume that any properly formed document will have no fully nested TOC field inside another TOC field.

Firstly the FieldStart nodes of each TOC are collected and stored. The specified TOC is then enumerated so all nodes within the field are visited and stored. The nodes are then removed from the document. Below code sample {demonstrates|shows} how to remove a specified TOC from a document.
