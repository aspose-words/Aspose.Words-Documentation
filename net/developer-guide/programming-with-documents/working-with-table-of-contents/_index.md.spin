...
Aspose.Words allows you to completely update a TOC with only several lines of code. This can be done to populate a newly inserted TOC or to update an existing TOC after changes to the document have been made. The following two methods must be used in order to update the TOC fields in the document:
...
|**Preserve New Line Entries** <br>*(\X Switch)* |<p>Similar to the switch above, this switch specifies that headings spanning over many lines (using new line characters not separate paragraphs) will be preserved as they are in the generated TOC. For example, a heading which is to spread across a few lines can use the new line character (Ctrl + Enter or ControlChar.LineBreak) to separate content across different lines. With this switch specified, the entry in the TOC will preserve these new line characters as shown below. <br><br>In this situation if the switch is not defined then the new line characters are converted to a single white space. </p><p>![working-with-table-of-contents-aspose-words](working-with-table-of-contents_10.png)</p>|
