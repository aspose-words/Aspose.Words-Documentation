...
Using the [Table.PreferredWidth](https://apireference.aspose.com/words/net/aspose.words.tables/table/properties/preferredwidth) property will adjust its preferred width relative to the container (i.e the page, text column or outer table cell). The code example given below {shows|demonstrates} how to set a table to auto fit to 50% of the page width.
...
Using the [CellFormat.PreferredWidth](https://apireference.aspose.com/words/net/aspose.words.tables/cellformat/properties/preferredwidth) property on a given cell will adjust its preferred width. Below example {shows|demonstrates} how to set the different preferred width settings.
...
The [Table.AllowAutoFit](https://apireference.aspose.com/words/net/aspose.words.tables/table/properties/allowautofit) property {enables|allows} cells in the table to grow and shrink to accommodate their contents. This property can be used in conjunction with a preferred cell width to format a cell which auto fits its content but which also has an initial width. The cell width can then grow past this width if needed. Below example {shows|demonstrates} how to set a table to shrink or grow each cell to accommodate its contents.
