...
- A Table contains {many|multiple|several|a few|numerous} Row nodes. A Table exposes all the normal members of a node which {allows|enables} you to freely move, modify and remove the table in the document.
- A Row represents a single row of a table and contains {many|multiple|several|a few|numerous} Cell nodes. Additionally a Row provides members which define how a row is displayed, for example the height and alignment.
...
- [DocumentBuilder.StartTable](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/starttable)
- [DocumentBuilder.InsertCell](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/insertcell)
- [DocumentBuilder.EndRow](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/endrow)
- [DocumentBuilder.EndTable](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/endtable)
- [DocumentBuilder.Writeln](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/writeln)
...
### Algorithm {for Creating|to Create} a Table {#algorithm-for-creating-a-table}

The basic algorithm for creating a table using DocumentBuilder is simple:

1. Start the table using [DocumentBuilder.StartTable](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/starttable).
1. Insert a cell using [DocumentBuilder.InsertCell](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/insertcell). This automatically starts a new row. If needed, use the [DocumentBuilder.CellFormat](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/properties/cellformat) property to specify cell formatting.
1. Insert cell contents using the DocumentBuilder methods.
1. Repeat steps 2 and 3 until the row is complete.
1. Call [DocumentBuilder.EndRow](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/endrow) to end the current row. If needed, use [DocumentBuilder.RowFormat](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/properties/rowformat) property to specify row formatting.
1. Repeat steps 2 - 5 until the table is complete.
1. Call [DocumentBuilder.EndTable](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/endtable) to finish the table building. The appropriate DocumentBuilder table creation methods are described below.
...
Use the DocumentBuilder.RowFormat property to specify row formatting. It returns a [RowFormat](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/properties/cellformat) object that represents all formatting for a table row.

#### End a Table

Call DocumentBuilder.EndTable to finish the current table. This method should be called only once after DocumentBuilder.EndRow was called. When called, DocumentBuilder.EndTable moves the cursor out of the current cell to a position just after the table. The following example {demonstrates|shows} how to build a formatted table that contains 2 rows and 2 columns.
...
Below example {shows|demonstrates} how to create a simple table using DocumentBuilder with default formatting.
...
Below example {shows|demonstrates} how to create a formatted table using DocumentBuilder.
...
Below example {shows|demonstrates} how to insert a nested table using DocumentBuilder.
...
Below example {shows|demonstrates} how to insert a table using the constructors of nodes.
...
Below example {shows|demonstrates} how to make a clone of the last row of a table and append it to the table. You can download the template file of this example from [here](https://github.com/aspose-words/Aspose.Words-for-.NET/blob/master/Examples/Data/Tables.docx).
...
Aspose.Words supports inserting content into a document from an HTML source by using the [DocumentBuilder.InsertHtml](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/inserthtml) method. The input can be a full HTML page or just a partial snippet. Using this method we can insert tables into our document by using table elements e.g &lt;table&gt;, &lt;tr&gt;, &lt;td&gt;. Below example {shows|demonstrates} how to insert a table in a document from a string containing HTML tags.
...
As described in previous articles, Aspose.Words provides {several|multiple|a few|many|numerous} methods for inserting new tables into a document. Each have their advantages and disadvantages, so often the choice of which to use depends on your situation. The table below can give you an idea of each technique.

|Method|Advantages|Disadvantages|
| :- | :- | :- |
|DocumentBuilder ([DocumentBuilder.StartTable](https://apireference.aspose.com/words/net/aspose.words/documentbuilder/methods/starttable))|Standard method of inserting tables and other document content.|Sometimes hard to create {many|multiple|several|a few|numerous} varieties of tables at the same time with the same instance of the builder.|
...
A Table like any other node in Aspose.Words has access to a Range object. Using this object, you can call methods over the entire table range to extract the table as plain text. The Range.Text property is used for this purpose. Below example {shows|demonstrates} how to print the text range of a table.
...
Below example {shows|demonstrates} how to print the text range of row and table elements.
