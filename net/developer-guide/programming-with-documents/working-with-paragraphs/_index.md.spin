...
The shading texture is set with a [TextureIndex](http://www.aspose.com/api/net/words/aspose.words/textureindex) enumeration value that {allows|enables} the application of various patterns to the **Shading** object. For example, to set a background color for a document element, use the [TextureIndex.TextureSolid](http://www.aspose.com/api/net/words/aspose.words/textureindex) value and set the foreground shading color as appropriate. Below example {shows|demonstrates} how to apply borders and shading to a paragraph.
