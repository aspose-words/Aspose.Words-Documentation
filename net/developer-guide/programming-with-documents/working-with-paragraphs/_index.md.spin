...
Both the border collection and each separate border have similar attributes like color, line style, line width, distance from text, and optional shadow. They are represented by properties of the same name. You can achieve different border types by combining the property values. In addition, both **BorderCollection** and **Border** objects allow you to reset these values to default by calling the [Border.ClearFormatting](https://apireference.aspose.com/words/net/aspose.words/border/methods/clearformatting) method. Note that when border properties are reset to default values, the border is invisible. The [Shading](https://apireference.aspose.com/words/net/aspose.words/shading) class contains shading attributes for document elements. You can set the desired shading texture and the colors that are applied to the background and foreground of the element.

The shading texture is set with a [TextureIndex](https://apireference.aspose.com/words/net/aspose.words/textureindex) enumeration value that {allows|enables} the application of various patterns to the **Shading** object. For example, to set a background color for a document element, use the [TextureIndex.TextureSolid](https://apireference.aspose.com/words/net/aspose.words/textureindex) value and set the foreground shading color as appropriate. Below example {shows|demonstrates} how to apply borders and shading to a paragraph.

Below example {shows|demonstrates} how to apply borders and shading to a paragraph.
