...
You can use SaveOptions.UpdateLastSavedTimeProperty property whether to update the corresponding built-in document property (BuiltInDocumentProperties.LastSavedTime) on document save. Following example {shows|demonstrates} how to update this property. 
...
When **Document.UpdateFields** or **Range.UpdateFields** is called all fields are updated over the entire document/range. This may involve building the document layout if a page-layout related field like the PAGE field is encountered during the update. Below example {shows|demonstrates} how to update all fields in a document. You can download the template file of this example from [here](https://github.com/aspose-words/Aspose.Words-for-.NET/blob/master/Examples/Data/Rendering.docx).
...
These changes to how fields are updated upon document layout are required and match how Microsoft Word updates fields. This now {allows|enables} a document to be rendered without any fields in the main body being updated which is how fields are evaluated in Microsoft Word.
...
As explained earlier, all fields are now updated using Document.UpdateFields. This now means a more concise and clearer way to update the fields in a document. This also means in situations such as updating a TOC field you no longer require any call to Document.UpdatePageLayout. All work is handled within the Document.UpdateFields call. Below examples {shows|demonstrates} how to completely rebuild TOC fields in the document by invoking field update.
...
Please note that this technique cannot be used properly on some fields in the header or footer. For example attempting to convert a PAGE field in a header or footer to static text will cause the same value to appear across all pages. This is because headers and footers are repeated across {multiple|several|a few|many|numerous} pages and when they remain as fields they are handled especially so they display the correct result for each page. However upon conversion, the field in the header is transformed into a static run of text. This run of text will be evaluated as if it is the last page in the section which will cause any of PAGE field in the header to display the last page over all pages.

Below code example {shows|demonstrates} how to replace the field with its most recent result.
...
The ConvertFieldsToStaticText method accepts two parameters, a CompositeNode and a FieldType enumeration. Being able to pass any composite node to this method {allows|enables} you to convert fields to static text in specific parts of your document only.
...
When passing a block level node such as a [Paragraph](https://apireference.aspose.com/words/net/aspose.words/paragraph) , be aware that in some cases fields can span across {multiple|several|a few|many|numerous} paragraphs. If this happens then it is recommended to instead pass the parent of the composite to avoid this.

The **FieldType** enumeration passed to the method specifies what type of field should be convert to static text. A field of any other type encountered in the document will be left unchanged. Below example {shows|demonstrates} how to convert all fields of a specified type in a document to static text. You can download template file of below examples from [here](https://github.com/aspose-words/Aspose.Words-for-.NET/blob/master/Examples/Data/Linked%20fields.docx).
...
Below example {shows|demonstrates} how to convert all fields of a specified type in a body of a document to static text.
...
Below example {shows|demonstrates} how to convert all fields of a specified type in a paragraph to static text.
