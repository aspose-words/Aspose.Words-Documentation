...
description: "Aspose.Words for .NET {enables|allows} you to specify various TrueType font sources: system folder, user sources, loading fonts from a stream, a file system or memory."
type: docs
weight: 30
...
The [FontSourceBase](https://apireference.aspose.com/words/net/aspose.words.fonts/fontsourcebase) class is used to specify various font sources. There are {several|multiple|a few|many|numerous} implementations of the **FontSourceBase** class:
...
On Windows, Aspose.Words first takes all available fonts from the _%windir%\Fonts folder. This setting will work for you most of the time. You only specify your own fonts folders if you need to. Aspose.Words for .NET also looks for additional fonts registered in the HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts registry key. In addition, Windows 10 {enables|allows} installation of fonts for the current user. Fonts are placed into the %userprofile%\AppData\Local\Microsoft\Windows\Fonts folder and also specified in the HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\Fonts registry, where Aspose.Words will look for these fonts.
...
Aspose.Words provides the [StreamFontSource](https://apireference.aspose.com/words/net/aspose.words.fonts/streamfontsource) class, which {allows|enables} loading fonts from the stream. To use the stream font source, a user needs to create a derived class from **StreamFontSource** and provide an implementation of the [OpenFontDataStream](https://apireference.aspose.com/words/net/aspose.words.fonts/streamfontsource/methods/openfontdatastream) method. The **OpenFontDataStream** method could be called {several|multiple|a few|many|numerous} times. For the first time, it will be called when Aspose.Words scans the provided font sources to get a list of available fonts. Later it may be called if the font is used in the document to parse the font data and to embed the font data to some output formats. **StreamFontSource** may be useful because it {allows|enables} loading the font data only when it is required, and not to store it in the memory for the [FontSettings](http://fontsettings/) lifetime.
