...
All the content in a document drawing layer is represented by the [Shape](https://apireference.aspose.com/words/net/aspose.words.drawing/shape) or [GroupShape](https://apireference.aspose.com/words/net/aspose.words.drawing/groupshape) node in the Aspose.Words Document Object Module (DOM). Such contents can be text boxes, images, AutoShapes, OLE objects, etc. Some fields are also imported as shapes, for example, the INCLUDEPICTURE field.

A simple image is represented by a **Shape** node of [ShapeType.Image](https://apireference.aspose.com/words/net/aspose.words.drawing/shapetype). This shape node has no child nodes but the image data contained within this shape node can be accessed by the [Shape.ImageData](https://apireference.aspose.com/words/net/aspose.words.drawing/shape/properties/imagedata) property. On the other hand, a shape can also be made up of {many|multiple|several|a few|numerous} child nodes. For instance, a text box shape, which is represented by the [ShapeType.TextBox](https://apireference.aspose.com/words/net/aspose.words.drawing/shapetype) property, can be made up of {many|multiple|several|a few|numerous} nodes, such as [Paragraph](https://apireference.aspose.com/words/net/aspose.words/paragraph) and [Table](https://apireference.aspose.com/words/net/aspose.words.tables/table). Most shapes can include the **Paragraph** and **Table** block-level nodes. These are the same nodes as those appearing in the main body. Shapes are always parts of some paragraph, either included directly inline or anchored to the **Paragraph,** but “floating” anywhere in the document page.

![rendering-shapes-separately-from-a-document_1](rendering-shapes-separately-from-a-document_1.png)

A document can also contain shapes which are grouped together. Grouping can be enabled in Microsoft Word by selecting {multiple|several|a few|many|numerous} objects and clicking “Group” in the right-click menu.
...
The [Save](https://apireference.aspose.com/words/net/aspose.words.rendering.noderendererbase/save/methods/1) method provides overloads that render a shape directly to a file or stream. Both overloads accept an instance of the [ImageSaveOptions](https://apireference.aspose.com/words/net/aspose.words.saving/imagesaveoptions) class, which {allows|enables} to define options for rendering the shape. This works in the same way as the [Document.Save](https://apireference.aspose.com/words/net/aspose.words/document/methods/save/index) method. Even though this parameter is required, you can pass a null value, specifying that there are no custom options.

The shape can be exported in any image format specified in the [SaveFormat](https://apireference.aspose.com/words/net/aspose.words/saveformat) enumeration. For example, the image can be rendered as a raster image, such as JPEG by specifying the [SaveFormat.Jpeg](https://apireference.aspose.com/words/net/aspose.words/saveformat) enumeration, or as a vector image, such as EMF by specifying the [SaveFormat.Emf](https://apireference.aspose.com/words/net/aspose.words/saveformat).
...
The **Shape** can be rendered to a certain scale using the [ShapeRenderer.RenderToScale](https://apireference.aspose.com/words/net/aspose.words.rendering/noderendererbase/methods/rendertoscale) method inherited from the [NodeRendererBase](https://apireference.aspose.com/words/net/aspose.words.rendering/noderendererbase) class. This is similar to the [Document.RenderToScale](https://apireference.aspose.com/words/net/aspose.words/document/methods/rendertoscale) method that accepts the same major parameters. The difference between these two methods is that with the **ShapeRenderer.RenderToScale** method, instead of a literal size, you choose a float value that scales the shape during its rendering. If the float value equals 1.0 causes the shape to be rendered at 100% of its original size. A float value of 0.5 will reduce the image size by half.

## Rendering a Shape Image

The [Shape](https://apireference.aspose.com/words/net/aspose.words.drawing/shape) class represents objects in the drawing layer, such as an AutoShape, text box, freeform, OLE object, ActiveX control, or a picture. Using the **Shape** class, you can create or modify shapes in a Microsoft Word document. An important property of a shape is its [ShapeType](https://apireference.aspose.com/words/net/aspose.words.drawing/shapebase/properties/shapetype). Shapes of different types can have different capabilities in a Word document. For example, only image and OLE shapes can have images inside them while most of the shapes can have text only.

The following example {shows|demonstrates} how to render a Shape image to a JPEG image separately from the document and save it to the disk:
...
The below example {shows|demonstrates} how to create a new Bitmap and Graphics object with the width and height of the shape to be rendered:
