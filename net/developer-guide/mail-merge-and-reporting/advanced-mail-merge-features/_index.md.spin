...
You can implement the [Next](https://apireference.aspose.com/words/net/aspose.words.fields/fieldnext) field to merge the next data record into the current resulting merged document, instead of starting a new merged document. It is used to display {multiple|several|a few|many|numerous} records in one document.
...
Aspose.Words {allows|enables} you to automatically map names of fields in your data source and names of mail merge fields in the template using the [MappedDataFields](https://apireference.aspose.com/words/net/aspose.words.mailmerging/mailmerge/properties/mappeddatafields) property. For example, if you have a field name called "Last Name" in your template, and in your data source you have the field name "Last Name" or another variation such as "Last_Name" or "LastName", then the field in the data source will automatically map to the corresponding mapped field. If a merge template is to be merged with {many|multiple|several|a few|numerous} data sources, mapped fields make it unnecessary to reenter the fields into the template to agree with the field names in the database.

The following code example {shows|demonstrates} how to add a mapped field using the [Add](https://apireference.aspose.com/words/net/aspose.words.mailmerging/mappeddatafieldcollection/methods/add) method when a merge field in a template and a data field in a data source have different names:
