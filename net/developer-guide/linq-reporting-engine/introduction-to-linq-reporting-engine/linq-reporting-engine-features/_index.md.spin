...
LINQ Reporting Engine {enables|allows} you to build reports in a similar way as Aspose.Words Mail Merge does. The engine provides the API that {enables|allows} you to populate template documents with data that comes from various sources.
...
In addition to traditional data sources, the engine {enables|allows} you to use custom ones. Custom data source objects are not treated in a special way by the engine. That is while working with such objects, you can use the same expression syntax as you use while writing C# code. Moreover, you can use accessible members of any custom type that can implement any complex logic. This feature makes the engine very flexible and suitable almost for any specific need while building a report.
