...
|Scenario|Comments|
| :- | :- |
|[DocumentBuilder.InsertImage](https://apireference.aspose.com/words/net/aspose.words.documentbuilder/insertimage/methods/3)|This method {allows|enables} you to pass a URL to insert an image from an external address. If the image is stored on a private network it may require authentication in order to load. The **ResourceLoadingCallback** can be used to pass the needed credentials |
...
This {enables|allows} control over where a document or node is exported to string when [HtmlSaveOptions.CssStyleSheetType](https://apireference.aspose.com/words/net/aspose.words.saving/htmlsaveoptions/properties/cssstylesheettype) is set to [CssStyleSheetType.External](https://apireference.aspose.com/words/net/aspose.words.saving/cssstylesheettype). This interface {allows|enables} control over where the external CSS is saved to, such as a stream.
