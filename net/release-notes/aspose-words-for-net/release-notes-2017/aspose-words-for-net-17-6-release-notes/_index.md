...
Upon inserting SVG image into the document, it is converted to meta-file (EMF).When do this in numerous threads exception might occur in GDI+. To prevent problems with multi-threading, we switched to our own EMF renderer instead of GDI+ used earlier.
