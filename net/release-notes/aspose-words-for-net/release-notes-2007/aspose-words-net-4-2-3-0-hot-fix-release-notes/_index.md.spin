...
- API documentation now shows values of enumerations. This is helpful for customers who use Aspose.Words from classical ASP.
  ...
  The size of generated files is optimized - if the same image occurs {multiple|several|a few|many|numerous} times in the document it is written to DOC, HTML and PDF files only once. 
