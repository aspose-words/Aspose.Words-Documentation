...
articleTitle: 在「文件屬性」中工作
linktitle: 在「文件屬性」中工作
...
「了解 API 和版本號直接寫入輸出文件的資訊很有用。」 例如，當將文件轉換為 PDF 時，Aspose.Words 會將「應用程式」欄位填寫為「Aspose.Words」，並將「PDF 製造商」欄位填寫為「Aspose.Words for .NET YY.M.N」，其中 *YY.M.N* 是用於轉化的 Aspose.Words 版本。 詳細資料請參閱 [Generator or Producer Name Included in Output Documents](/words/net/generator-or-producer-name-included-in-output-documents/)。
...
在 Microsoft Word 中，您可以使用「檔案」→「屬性」選單來存取檔案屬性。
