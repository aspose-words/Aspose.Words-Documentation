...
「Microsoft Word 文檔可能包含各種特殊含義的字符。」 通常它們是用來做格式化的，在正常模式下不會被繪製。 你可以透過「標準」工具列上的「顯示/隱藏格式標記」按鈕來使它們顯露。

有時您可能需要新增或移除文字中的字元 。 例如，當從文件中程式化獲取文字時，「Aspose.Words」會保存大多數控制字元，因此如果你需要處理這些文字，你應該將字元移除或替換。
