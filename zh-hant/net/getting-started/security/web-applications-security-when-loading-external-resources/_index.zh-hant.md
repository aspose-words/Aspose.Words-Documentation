...
如果攻擊者能夠說服使用者或伺服器處理包含指向他們控制的主機之資源連結的文件，則該攻擊者將以 NTLM 密碼格式接收使用者或服務帳戶的憑證。 這樣的資料可以被重新使用在一個经典的「傳送密碼」攻擊中，讓攻擊者能以受害者用戶或服務帳戶的身份取得任何資源的存取權。
...
攻擊者可以上傳一篇文件，該文件會引用或包含極大尺寸的圖形-即所謂「解壓炸彈」。 「當處理這些圖像時，該圖書館會消耗大量記憶體與中央處理器時間。」
