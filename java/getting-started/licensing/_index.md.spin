...
The evaluation version is the same as the purchased one – the Trial version simply becomes licensed when you add {a few|multiple|several|many|numerous} lines of code to apply the license.

The Trial version of Aspose.Words for Java and Aspose.Words for Android via Java without the specified license provides full product functionality, but inserts an evaluative watermark at the top of the document upon loading and saving and limits the maximum document size to {a few|multiple|several|many|numerous} hundred paragraphs.
...
The following code example {shows|demonstrates} how to initialize a license from a stream using another [SetLicense](https://apireference.aspose.com/words/java/com.aspose.words/license#setLicense(java.io.InputStream)) method:
...
* Use the Fully Qualified License Class Name. Each Aspose product has a License class in its own namespace. For example, Aspose.Words has **com.aspose.words.License** and Aspose.Cells has com.aspose.cells.License class. Using the fully qualified class name {allows|enables} you to avoid confusion as to which license applies to which product.
