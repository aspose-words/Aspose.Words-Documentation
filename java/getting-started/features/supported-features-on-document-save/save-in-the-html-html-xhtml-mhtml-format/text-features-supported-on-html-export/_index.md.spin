...
Each Run node in the model is exported as a separate span to retain formatting properly. Some documents can contain {many|multiple|several|a few|numerous} runs that are unrequired and can be joined. In the resulting HTML document, this can result in {many|multiple|several|a few|numerous} extra span elements.
...
There is an option to control how size is exported. Font can be exported as points or as em units. This {allows|enables} fonts to be resized automatically by browsers by increasing or decreasing the font size.
