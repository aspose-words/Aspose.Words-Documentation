...
All of the following image types listed in the table below this overview are supported. When a document contains many references to the same image from an external address (e.g the internet) then the image is only downloaded once.
...
|**Feature**|**Supported**|**Comment**|**See Also**|
| :- | :- | :- | :- |
|Linked|Yes|Note that Aspose.Words cannot update an OLE link. However, you can modify a link only OLE object to point to a new location which can provide a partial way of changing OLE objects. <br><br>When such a document is saved and opened in Microsoft Word it will detect the change in the link and prompt the user to update the linked object. This results in the linked object appearing in the document. <br><br>Such a technique will only work for Word formats and if the document is opened in an editor which enables updating OLE objects (such as Microsoft Word). If the link is changed and the document to a different format such as PDF then the original content will still appear in the output.|<p>- **OleFormat.IsLink**</p><p>- **OleFormat.SourceFullName**</p>|
|Embedded|Yes| | |
|Draw Aspect|Yes| | |
...
|SmartArt (VML)|Yes|Represented as a group shape with child shapes representing the different elements. <br><br>You can add, modify or remove parts of the smart art. You can also extract the plain text content.| |
...
|Polygon Wrap Points|Planned|This property is currently lost upon import of Word documents. <br><br>This feature will be included in a future version.| |
