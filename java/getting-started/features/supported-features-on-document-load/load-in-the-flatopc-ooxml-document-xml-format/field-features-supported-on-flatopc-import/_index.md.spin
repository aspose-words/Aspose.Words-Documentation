...
Fields are place holders in the document which can be dynamically updated to display new information. The most common type of fields is MergeFields and Page fields. The first {allows|enables} you to merge data into a document, the latter displays the current page number of the page where the field appears on.
...
We provide the Field facade for working with this structure more easily. This {allows|enables} you to easily find the field code and field result of a field. Currently, you can only retrieve this facade while inserting a new field into the document, there are plans to introduce a new field API which {allows|enables} you to get this facade from any field the document.
...
|FileSize|Yes|This field is updated with "0" file length value when the document is loaded from the stream.| |
...
|TOC (Table of Contents)|Yes|<p>Table of Contents is fully supported in Aspose.Words. You can create a TOC from scratch and update it. The Aspose.Words field engine produces a TOC that looks just like how Microsoft Word does. <br><br>The TOC can be generated from styles in the document and from TC fields. <br><br>The following switches on the TOC field are supported:</p><p>- Heading Styles (\O)</p><p>- Outline Levels (\U)</p><p>- Custom Styles (\T)</p><p>- Bookmarked Content (\B)</p><p>- Use TC Fields (\F and \L)</p><p>- Omit Page Numbers (\N)</p><p>- Insert As Hyperlinks (\H)</p><p>- Set Separator Character (\P)</p><p>- Preserve Tab Entries (\W)</p><p>- Preserve New Line Entries (\X) <br><br>  All other switches will be supported in future versions. <br><br>  Currently mixed number formats in TOC is unsupported during field update and will appear as Arabic numbering. This will be improved in a future version. These number formats are supported in other types of fields during update. Chapter numbering is also unsupported at the moment.</p>| |
...
|Bibliography|Yes|This field is not yet supported during a field update.| |
...
The mail merge engine {allows|enables} you to quickly merge a variety of data into a document. You can merge simple data using simple mail merge as well as complex relational data from a database using merge regions. Both are easy to achieve using Aspose.Words.
...
|GreetingLine|Yes| | |
|If|Yes| | |
|MergeField|Yes| |- [How to Rename Merge Fields](/words/java/update-field/#updatefield-howtorenamemergefields)|
|MergeRec|Yes| | |
|MergeSeq|Yes| | |
|Next|Yes| | |
|NextIf|Yes| | |
...
|**Feature**|**Supported**|**Comment**|**See Also**|
| :- | :- | :- | :- |
|Date and Time Formatting|Yes|All date formats (pictures defined by the @ tag, e.g @ dd/MM/yyyy) are supported during the update. <br><br>Hijri, Lunar or Saka Era calendar are not supported and are updated as regular Gregorian dates.| |
