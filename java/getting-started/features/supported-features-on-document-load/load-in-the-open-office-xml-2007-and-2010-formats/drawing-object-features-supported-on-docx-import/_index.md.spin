...
type: docs
weight: 40
url: /java/drawing-object-features-supported-on-docx-import/
---
...
|**Feature**|**Supported**|**Comment**|**See Also**|
| :- | :- | :- | :- |
|Linked |Yes |Note that Aspose.Words cannot update an OLE link. However you can modify a link only OLE object to point to a new location which can provide a partial way of changing OLE objects. <br><br>When such a document is saved and opened in Microsoft Word it will detect the change in the link and prompt the user to update the linked object. This results in the linked object appearing in the document. <br><br>Such a technique will only work for Word formats and if the document is opened in an editor which {allows|enables} to update OLE objects (such as Microsoft Word). If the link is changed and the document to a different format such as PDF then the original content will still appear in the output. |<p>- **OleFormat.IsLink** </p><p>- **OleFormat.SourceFullName**</p>|
|Embedded |Yes | | |
|Draw Aspect |Yes | | |
...
|Watermark |Yes |A watermark in a Microsoft Word document is actually a text shape or image centered in the middle of the page but in the header or footer. This {allows|enables} the watermark to appear behind all content and as faded. |- [How to Add a Watermark to a Document]()|
...
|Scale |Yes |There is currently no way to access the scale of Shape loaded in Aspose.Words. <br><br>The size of the shape after scale is applied is calculated and stored as the public size of the Shape. This size can be found using the Shape class. |- **Shape.SizeInPoints**|
|Relative Size |Yes | | |
