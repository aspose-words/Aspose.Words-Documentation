...
Currently, images are only exported as absolute size (points) and not in per cent. This means that images will not resize when the "Increase Text/Decrease Text" buttons are pressed. These buttons are common in browsers and eReaders.
...
|Horizontal Alignment|Planned|Currently exported as absolute position. This will be supported in a future version. <br><br>An alternative to achieve the same effect is to make the image inline in a paragraph and set the paragraph alignment to centre.| |
