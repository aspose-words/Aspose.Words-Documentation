...
Any document loaded into Aspose.Words can be converted to PDF that conforms one of following compliance level.
...
|Built-In Properties |Yes |Built-in properties such as word and character count are updated using Aspose.Words but are not updated automatically on save. <br><br>Instead you need to explictly update these properties using the appropriate Document member. We will add automatic update of these properties in a future version. <br><br>Title, Author, Subject, Keywords is output to PDF. |- **Document.UpdateWordCount**|
