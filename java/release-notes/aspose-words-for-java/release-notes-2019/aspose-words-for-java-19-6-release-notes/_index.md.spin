...
- Fixed exception when table which spans {multiple|several|a few|many|numerous} pages and has {many|multiple|several|a few|numerous} nested floaters is broken across pages.
- Improved text positioning when there is a paragraph-relative shape with an anchor wrapped around {multiple|several|a few|many|numerous} floaters.
...
Obsolete method Replace was removed from Range class. And fixed {a few|multiple|several|many|numerous} minor bugs in new FindReplacer.
