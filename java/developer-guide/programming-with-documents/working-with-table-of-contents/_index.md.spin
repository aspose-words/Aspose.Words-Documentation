...
Often you will work with documents containing a table of contents (TOC). Using Aspose.Words you can insert your own table of contents or completely rebuild the existing table of contents in the document using just {a few|multiple|several|many|numerous} lines of code.
...
A table of contents in a Word document can be built in {several|multiple|a few|many|numerous} ways and formatted using a variety of options. The field switches that you pass to the method control the way the table is built and displayed in your document.
...
The following code example {shows|demonstrates} how to insert a Table of Contents field into a document.
...
The following code example {demonstrates|shows} how to insert a Table of contents (TOC) into a document using heading styles as entries.
...
Aspose.Words {allows|enables} you to completely update a TOC with only {a few|multiple|several|many|numerous} lines of code. This can be done to populate a newly inserted TOC or to update an existing TOC after changes to the document have been made.
...
The following code example {shows|demonstrates} how to completely rebuild TOC fields in the document by invoking field updates.
...
![ccontrol-table-of-contents-field-aspose-words-java](how-to-insert-and-work-with-the-table-of-contents-field_2.png)
...
The following code example {shows|demonstrates} how to insert a TC field into the document using [DocumentBuilder](https://apireference.aspose.com/words/java/com.aspose.words/DocumentBuilder).
...
The following code example {shows|demonstrates} how to modify the position of the right tab stop in TOC related paragraphs.
...
The code below {demonstrates|shows} this. The removal of the TOC field is simpler than a normal field as we do not keep track of nested fields. Instead, we check the FieldEnd node is of type FieldType.FieldTOC which means we have encountered the end of the current TOC. This technique can be used in this case without worrying about any nested fields as we can assume that any properly formed document will have no fully nested TOC field inside another TOC field.

Firstly the FieldStart nodes of each TOC are collected and stored. The specified TOC is then enumerated so all nodes within the field are visited and stored. The nodes are then removed from the document. The following code example {demonstrates|shows} how to remove a specified TOC from a document.
