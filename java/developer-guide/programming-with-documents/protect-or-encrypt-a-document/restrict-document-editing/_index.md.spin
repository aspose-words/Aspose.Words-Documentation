...
description: "Aspose.Words for Java {allows|enables} to restrict editing a document by setting a restriction type. You can also remove protection and make unrestricted editable regions."
type: docs
weight: 30
...
Aspose.Words {allows|enables} you to control the way you restrict the content using the [ProtectionType](https://apireference.aspose.com/words/java/com.aspose.words/ProtectionType) enumeration parameter. This will enable you to select an exact type of protection such as the following:
...
Note that the password being set is just a property in a document that can be removed if the document properties are accessed. Accordingly, such a password is not a guarantee of the document security. The [Unprotect](https://apireference.aspose.com/words/java/com.aspose.words/document#unprotect()) method {shows|demonstrates} just that.
...
Aspose.Words {allows|enables} you to protect your documents from changes using the [Protect](https://apireference.aspose.com/words/java/com.aspose.words/document#protect(int)) method. This method is not a security feature and does not encrypt a document.
