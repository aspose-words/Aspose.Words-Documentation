...
In Aspose.Words this is found under the [RowFormat](https://apireference.aspose.com/words//java/com.aspose.words/rowformat) object of a [Row](https://apireference.aspose.com/words//java/com.aspose.words/row) as the property [RowFormat.getAllowBreakAcrossPages()](https://apireference.aspose.com/words/java/com.aspose.words/rowformat#AllowBreakAcrossPages). Below example {shows|demonstrates} how to disable rows breaking across pages for every row in a table.
...
To stop a table from splitting across the page we need to state that we wish the content contained within the table to stay together. In Microsoft Word this involves selecting the table and enabling “Keep with Next” under Paragraph Format. In Aspose.Words the technique is the same. Each paragraph inside the cells of the table should have [ParagraphFormat.setKeepWithNext(boolean)](https://apireference.aspose.com/words/java/com.aspose.words/paragraphformat#KeepWithNext) set to true. The exception is the last paragraph in the table which should be set to false. Below example {shows|demonstrates} how to set a table to stay together on the same page.
