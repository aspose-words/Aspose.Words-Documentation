...
Aspose.Words can import list item of a text file as list numbers or plain text in its document object model. The TxtLoadOptions.DetectNumberingWithWhitespaces property {allows|enables} to specify how numbered list items are recognized when a document is imported from plain text format. If this option is set to *true*, whitespaces are also used as list number delimiters: list recognition algorithm for Arabic style numbering (1., 1.1.2.) uses both whitespaces and dot (".") symbols.
...
You can control the way of handling leading and trailing spaces during loading TXT files. The leading spaces could be trimmed, preserved or converted to indent and trailing spaces could be trimmed or preserved. The code example given below {shows|demonstrates} how to trim leading and trailing spaces while importing the TXT file.
...
The following code example {demonstrates|shows} how to detect the text direction of the document while importing the TXT file.
