...
Asian Typography tab of the Paragraph properties dialog in MS Word has a line break group. The options of this group can be set using FarEastLineBreakControl, WordWrap, HangingPunctuation properties of ParagraphFormat class. The code example given below {shows|demonstrates} how to use these properties.
...
The shading texture is set with a TextureIndex enumeration value that {allows|enables} the application of various patterns to the **Shading** object. For example, to set a background color for a document element, use the TextureIndex.TextureSolid value and set the foreground shading color as appropriate. The code example given below {shows|demonstrates} how to apply borders and shading to a paragraph.

The code example given below {shows|demonstrates} how to apply borders and shading to a paragraph.
