...
LINQ Reporting Engine provides {several|multiple|a few|many|numerous} additional number formats that can not be specified using format strings because these formats are missing in standard  REF platform Java libraries. The following table describes these formats.
...
LINQ Reporting Engine provides {several|multiple|a few|many|numerous} additional string formats that can not be specified using format strings because these formats are missing in standard  REF platform Java libraries. The following table describes these formats.
