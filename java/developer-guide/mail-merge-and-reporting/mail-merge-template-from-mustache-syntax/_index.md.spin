...
type: docs
description: "Aspose.Words for Java {allows|enables} you to create templates with mustache syntax – an alternative variant of syntax that consists of tag names enclosed by {{ }}. Mustache syntax is the only option to use with templates that do not contain fields (HTML or TXT). With Word templates, you have two options: fields or Mustache syntax."
...
Aspose.Words {allows|enables} you to create templates with mustache syntax in addition to the familiar templates. A Mustache is an alternative variant of template syntax that consists of tag names enclosed by {{ }} and is backed by a model object that contains the data for the template.
...
The following code example {shows|demonstrates} how to replace the Mustache tags with specific data:
...
Aspose.Words {allows|enables} you to use mail merge fields and Mustache tags with the IF statement. The IF fields can be used in any mail merge document to suppress undesired spaces and commas if a field is blank.
