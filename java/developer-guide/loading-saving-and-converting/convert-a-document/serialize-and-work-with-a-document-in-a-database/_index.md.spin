...
description: "Aspose.Words for Java {allows|enables} you to convert a document into a byte array for work with this document in a database. You can store and retrieve a document to and from the database."
aliases:
...
The example below {shows|demonstrates} how to serialize a **Document** object to obtain a byte array, and then how to unserialize the byte array to obtain a **Document** object again:
...
This section {shows|demonstrates} how to save a document in a database and then load it back into a Document object for working with it. For simplicity, the file name is the key used to store and fetch documents from the database. The database contains two columns. The first column “FileName” is stored as a String and is used to identify documents. The second column “FileContent” is stored as a BLOB object which stores the document object in the byte form.

The following code example {shows|demonstrates} how to set up a connection to a database and execute commands:
...
The following code example {shows|demonstrates} how to save a document to the database, then read the same document again, and finally delete the record containing the document from the database:
...
The following code example {shows|demonstrates} how to delete a document from the database, using the file name to fetch the record:
