...
When rendering reports in Flow layout, it is very important to control how page breaks are inserted into the document. Aspose.Words for Reporting Services offers many modes each of which has its own algorithm of page breaks placement. If you are not satisfied with the default mode, you can try the others to find the best balance between fidelity and editability of the resulting Word document. 
...
**Page break on each page.** In this mode, a page break is inserted at the end of each page. This normally provides the best precision of the export (the report exported to a Word document will look very close to that exported to PDF), but makes the document harder to edit because of too numerous page breaks inserted. 
